...
public class email_check extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String email = jc.getJSONFromAjax(request.getReader());
        EditSimpleUserTable simpleUser = new EditSimpleUserTable();
        EditDoctorTable doctor = new EditDoctorTable();
        String new_email = email.substring(10, email.length() - 2);

        try {

            if (simpleUser.emailInDataBase(new_email) == false && doctor.emailInDataBase(new_email) == false) {
