    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (eut.usernameInDataBase(obj.get("username").getAsString())) {
                ...
                ArrayList<Randevouz> ra = ert.databaseToDoctorRandevouz(doctor.get("doctor_id").getAsInt());
                for (Randevouz a : ra) {
                    ...
                    if (Date2.equals(Date)) {
                        ...
                        if ((d1.getSeconds() / 60) > 0 && (d1.getSeconds() / 60) < 30) {
                            response.setStatus(403);
                            response.getWriter().write("Yparxei rantevou pou einai ligo pio meta apo auto");
                            return;
                        } else if ((d1.getSeconds() / 60) < 0 && (d1.getSeconds() / 60) > -30) {
                            response.setStatus(403);
                            response.getWriter().write("Yparxei rantevou pou einai ligo pio prin apo auto");
                            return;
                        }
